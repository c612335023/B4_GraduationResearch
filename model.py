...
class MusicTransformer(torch.nn.Module):
    ...
    def generate(self,
        ...
        for i in Bar('generating').iter(range(length)):
            inter = torch.where(result_array == config.pad_token)[1]

            _, _, look_ahead_mask = \
                utils.get_masked_with_pad_tensor(decode_array.size(1), decode_array, decode_array, pad_token=config.pad_token, mask_token=inter)
            result, _ = self.Decoder(decode_array, look_ahead_mask)
            ...
            pdf = dist.OneHotCategorical(probs=result[:, inter[0]-1])   # change
            result = pdf.sample().argmax(-1).unsqueeze(-1)
            decode_array[0,inter[0]] = result
            result_array[0,inter[0]] = result
            del look_ahead_mask
