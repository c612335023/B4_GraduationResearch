...
def get_masked_with_pad_tensor(size, src, trg, pad_token, mask_token=None):
    """
    ...
    if trg is not None:
        ...
        if mask_token == None:
            seq_mask[:, -int(trg.size()[-1]/4):] = False
        else:
            start, end = mask_token[0], mask_token[-1]+1
            seq_mask[:, :start] = False
            seq_mask[:, start:end] = True
            seq_mask[:, end:] = False
